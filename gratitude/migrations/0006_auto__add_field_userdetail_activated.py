...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding field 'UserDetail.activated'
        db.add_column('gratitude_userdetail', 'activated',
                      self.gf('django.db.models.fields.DateTimeField')(null=True),
                      keep_default=False)
        if not db.dry_run:
            for userDetail in orm.UserDetail.objects.all():
                user = User.objects.all().filter(id = userDetail.user.id)[0]
                userDetail.activated = user.date_joined
                userDetail.save()

    def backwards(self, orm):
        # Deleting field 'UserDetail.activated'
        db.delete_column('gratitude_userdetail', 'activated')
